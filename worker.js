...
process.on('message', ({
  ...
  (async() => {
    let browser;
    const resultRow = [target[`${inputColumnsHeader[0]}`], target[`${inputColumnsHeader[1]}`], target[`${inputColumnsHeader[2]}`]];
    const resultObject = {
      data:  undefined,
      error: undefined,
      target,
    ...
    try {
      ...
      if (outputFormat === supportedOutputStreamTypes[0]) {
        const errorPairs = [resultRow[0], resultRow[1], resultRow[2]];
      ...
      if (outputFormat === supportedOutputStreamTypes[0]) {
        ...
        arr = arr.map(function(str) {
          var tempStr = str + '';
            //return escape(tempStr.split(' ').join('/'));
            return tempStr.replace(/[^\x20-\x7E]/g, '');
        }); 
        resultObject.data = csvStringify([arr], { delimiter: ',' });
      } else if (outputFormat === supportedOutputStreamTypes[1]) {
...
const jsObjectResultGenerator = ({ resultRow, reportHeader, orderMap }) => {
  ...
  jsObject[reportHeader[2]] = resultRow[2];
  let auditObject = {};
