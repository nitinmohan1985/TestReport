...
writeStream.on('open', () => {
  ...
  lighthouseAuditing.on('data', ({ data, progress, header }) => {
    const { device, url, site, totalTasksNum, leftTasksNum } = progress;
    consoleLogger(program.logMode && !header, `Device = ${device} || URL = ${url} || Site = ${site}`); 
...
var fileFormatDetails=[];
fileFormatDetails.push('csv');
fileFormatDetails.push('json')
//fileFormatDetails.push('xlsx')
for(i=0;i<fileFormatDetails.length;i++)
{
  outputFileFormat=fileFormatDetails[i];
  console.log(outputFileFormat);
